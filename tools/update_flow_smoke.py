...
a fake page, so the decision table is verified without network or GUI.
    ...
    patch release + "install patches automatically" on the Windows build
...
ZIP = ("UniDocs-linux-x86_64.zip", "https://example.invalid/zip")
...
def run_check(
    ...
    *,
    ...
    """Run one startup check and return (overlay, config)."""
    page = FakePage()
...
print("patch release on Linux (no self-update) -> asks instead:")
overlay, _cfg, _ = run_check("2.3.1", "2.3.0", asset=ZIP, self_update=False)
prompt = overlay[0] if overlay else None
